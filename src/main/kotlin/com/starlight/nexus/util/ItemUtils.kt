...
import com.starlight.nexus.util.nms.MinecraftReflection
import com.starlight.nexus.util.nms.NBTUtil
...
object ItemUtils {
    ...
    private val AS_NMS_COPY_METHOD: Method =
        Reflection.getMethodSuppressed(CRAFT_ITEM_STACK_CLASS, "asNMSCopy", ItemStack::class.java)!!
    private val AS_BUKKIT_COPY_METHOD: Method =
        Reflection.getMethodSuppressed(CRAFT_ITEM_STACK_CLASS, "asBukkitCopy", ITEM_STACK_CLASS)!!
    ...
    private val ARMOR_EQUIPMENT_SLOTS =
    ...
    fun applySkullTexture(itemStack: ItemStack, texture: String): ItemStack {
        try {
            ...
            val skullOwner = NBTUtil.getCompound(tag, "SkullOwner")
            NBTUtil.setString(skullOwner, "Id", UUID.randomUUID().toString())

            val textureValueTag = NBTUtil.newTag()
            NBTUtil.setString(textureValueTag, "Value", texture)

            val textures = NBTUtil.newList()
            NBTUtil.addToList(textures, textureValueTag)

            val properties = NBTUtil.getCompound(skullOwner, "Properties")
            NBTUtil.set(properties, "textures", textures)
            NBTUtil.set(skullOwner, "Properties", properties)
            NBTUtil.set(tag, "SkullOwner", skullOwner)
            ...
            e.printStackTrace()
            throw RuntimeException("Error applying skull texture", e)
        }
    }

    @JvmStatic
    fun buildHeadByValue(itemStack: ItemStack, value: String): ItemStack {
        val hashAsId = UUID(value.hashCode().toLong(), value.hashCode().toLong())
        val skull = Bukkit.getUnsafe().modifyItemStack(
            ItemStack(Material.SKULL_ITEM, 1, 3.toShort()),
            "{SkullOwner:{Id:\"$hashAsId\",Properties:{textures:[{Value:\"$value\"}]}}}"
        )
        skull.durability = 3

        val itemMeta = skull.itemMeta ?: Bukkit.getItemFactory().getItemMeta(Material.SKULL_ITEM)
        itemMeta.displayName = itemStack.itemMeta.displayName
    ...
    @JvmStatic
    fun addToItemTag(itemStack: ItemStack, pairs: List<Pair<String, Any>>, preserve: Boolean = false): ItemStack {
        val nmsCopy = getNmsCopy(itemStack)
        val tag = NBTUtil.getOrCreateTag(nmsCopy)

        for (pair in pairs) {
            when (pair.second) {
                is String -> NBTUtil.setString(tag, pair.first, pair.second as String)
                is Int -> NBTUtil.setInt(tag, pair.first, pair.second as Int)
                is Long -> NBTUtil.setLong(tag, pair.first, pair.second as Long)
                is Double -> NBTUtil.setDouble(tag, pair.first, pair.second as Double)
                is Float -> NBTUtil.setFloat(tag, pair.first, pair.second as Float)
                is UUID -> NBTUtil.setUUID(tag, pair.first, pair.second as UUID)
                else -> NBTUtil.setString(
                    tag,
                    pair.first,
                    pair.second.toString()
                ) // Default to toString if not supported
            }
