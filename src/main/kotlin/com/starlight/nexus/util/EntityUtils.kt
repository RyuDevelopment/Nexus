...
object EntityUtils {
    ...
    private val displayNames = EnumMap<EntityType, String>(EntityType::class.java)
    private var currentFakeEntityId = -1

    fun getFakeEntityId(): Int {
    ...
    fun parse(input: String): EntityType? {
        for ((key, value) in displayNames) {
            if (value.replace(" ", "", true) == input.trim()) {
                return key
            }
        }
