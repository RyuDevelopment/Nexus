package com.starlight.nexus.command.bukkit

import com.starlight.nexus.command.CommandCompat
import com.starlight.nexus.command.CommandHandler
import com.starlight.nexus.command.CommandNode
import com.starlight.nexus.command.data.argument.ArgumentProcessor
...
class ExtendedCommand(val node: CommandNode, private val plugin: Plugin) :
    Command(node.name, "", "/", node.getRealAliases().toList()), PluginIdentifiableCommand {
    ...
    fun tabComplete(sender: CommandSender, cmdLine: String): List<String> {
        ...
        if (rawArgs.isEmpty()) {
            ...
            if (realNode.hasCommands()) {
                ...
                completions.addAll(
                    realNode.children.values.stream().filter { node ->
                        node.name != null && node.canUse(sender) && (StringUtils.startsWithIgnoreCase(
                            node.name,
                            name
                        ) || StringUtils.isEmpty(name))
                    }.map { node -> node.name }.collect(Collectors.toList<String>())
                )
            ...
            if (realNode.validFlags.isNotEmpty()) {
                for (flag in realNode.validFlags) {
                    ...
                    if (Flag.FLAG_PATTERN.matcher(arg).matches() || arg == "-" && (StringUtils.startsWithIgnoreCase(
                            flag,
            ...
            try {
                val params = realNode.parameters.stream().filter { param -> param is ParameterData }
                    .map { param -> param as ParameterData }.collect(Collectors.toList())

                val fixed = 0.coerceAtLeast(currentIndex)

                if (params.isEmpty()) {
                    return emptyList()
                }

                val data: ParameterData = params.getOrNull(fixed) ?: return emptyList()
                val parameterType: ParameterType<*>? = CommandHandler.getParameterType(data.type)
                ...
                if (parameterType != null) {
                    ...
                    val argumentBeingCompleted =
                        (if (currentIndex >= realArgs.size || realArgs.isEmpty()) "" else realArgs[currentIndex]).trim()
                    val suggested = parameterType.tabComplete(sender, data.tabCompleteFlags, argumentBeingCompleted)

                    completions.addAll(
                        suggested.stream().filter { s -> StringUtils.startsWithIgnoreCase(s, argumentBeingCompleted) }
